...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Ambil ID pengajar dari data_pengajar
    $stmt_get_id = $koneksi->prepare("SELECT id FROM data_pengajar WHERE user_id = ?");
    $stmt_get_id->bind_param("i", $user_id_login);
    $stmt_get_id->execute();
    $id_pengajar_db = $stmt_get_id->get_result()->fetch_assoc()['id'];
    $stmt_get_id->close();
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] == 0) {
        $upload_dir = '../uploads/';
        $filename = 'pengajar_' . $id_pengajar_db . '_' . time() . '_' . basename($_FILES['foto']['name']);
    ...
    try {
        // --- UPDATE TABEL data_pengajar ---
        $sql_update_pengajar = "UPDATE data_pengajar SET 
                                nama_lengkap = ?, nik = ?, no_kk = ?, jenis_kelamin = ?, 
                                tempat_lahir = ?, tanggal_lahir = ?, alamat = ?, 
                                no_hp = ?, riwayat_pendidikan = ? 
                                $foto_sql_part 
                              WHERE id = ?";
        
        // --- INI ADALAH BAGIAN YANG DIPERBAIKI ---
        
        // 1. Tentukan tipe data untuk 9 field pertama (semua string 's')
        $types = "sssssssss"; 
        
        // 2. Siapkan 9 parameter pertama
        $params = [
            $nama_lengkap, $nik, $no_kk, $jenis_kelamin, 
            $tempat_lahir, $tanggal_lahir, $alamat, 
            $no_hp, $riwayat_pendidikan
        ...
        $params[] = $id_pengajar_db; // Tambah nilai ID ke array
        
        // --- AKHIR PERBAIKAN ---

        $stmt_update = $koneksi->prepare($sql_update_pengajar);
        // Sekarang $types dan $params pasti cocok
        ...
        header("Location: biodata_pengajar.php?status=sukses");
        exit();

    } catch (Exception $e) {
        $koneksi->rollback();
        // Redirect kembali ke halaman edit dengan pesan error
        header("Location: edit_biodata_pengajar.php?status=gagal_db&error=" . urlencode($e->getMessage()));
        exit();
    }
}
...
<head>
    ...
    <title>Edit Biodata Pengajar</title>
    ...
    <style>
        /* (Salin semua CSS dari jawaban Anda sebelumnya, sudah responsif) */
        :root {
            --warna-hijau: #00a86b;
            --warna-latar: #f4f7f6;
            --warna-teks: #333333;
    ...
    <div class="biodata-container">
        ...
        <form method="POST" enctype="multipart/form-data" id="edit-form">
            ...
            <header class="profile-header">
                <a href="biodata_pengajar.php" class="back-btn">
                    ...
                    <label for="foto-upload">
                        <div class="profile-pic">
                            <?php if (!empty($pengajar_data['foto'])): ?>
                                <img src="../uploads/<?php echo htmlspecialchars($pengajar_data['foto']); ?>" alt="Foto Profil" id="profile-image-preview">
                            <?php else: ?>
                                <div class="icon" id="profile-icon-preview"><i class="fas fa-user-tie"></i></div>
                ...
                <h2><?php echo htmlspecialchars($pengajar_data['nama_lengkap']); ?></h2>
                <div class="nis-badge">
                    ID = <?php echo htmlspecialchars($pengajar_data['nip']); ?>
                </div>
            </header>
                    ...
                    <div class="card-body">
                        <div class="form-grid">
                            <div class="form-group">
                                ...
                                <input type="text" id="nik" name="nik" value="<?php echo htmlspecialchars($pengajar_data['nik']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="no_kk">No. KK</label>
                                <input type="text" id="no_kk" name="no_kk" value="<?php echo htmlspecialchars($pengajar_data['no_kk']); ?>">
                            </div>
                            <div class="form-group required">
                                <label for="nama_lengkap">Nama Lengkap</label>
                                <input type="text" id="nama_lengkap" name="nama_lengkap" value="<?php echo htmlspecialchars($pengajar_data['nama_lengkap']); ?>" required>
                            </div>
                            <div class="form-group">
                                <label for="jenis_kelamin">Jenis Kelamin</label>
                                <select id="jenis_kelamin" name="jenis_kelamin">
                                    <option value="Laki-laki" <?php if($pengajar_data['jenis_kelamin'] == 'Laki-laki') echo 'selected'; ?>>Laki-laki</option>
                                    <option value="Perempuan" <?php if($pengajar_data['jenis_kelamin'] == 'Perempuan') echo 'selected'; ?>>Perempuan</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="tempat_lahir">Tempat Lahir</label>
                                <input type="text" id="tempat_lahir" name="tempat_lahir" value="<?php echo htmlspecialchars($pengajar_data['tempat_lahir']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="tanggal_lahir">Tanggal Lahir</label>
                                <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="<?php echo htmlspecialchars($pengajar_data['tanggal_lahir']); ?>">
                            </div>
                            <div class="form-group">
                                <label for="no_hp">No. Hp</label>
                                <input type="text" id="no_hp" name="no_hp" value="<?php echo htmlspecialchars($pengajar_data['no_hp']); ?>">
                            </div>
                            <div class="form-group required">
                                <label for="email">Email</label>
                                <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($pengajar_data['email']); ?>" required>
                            </div>
                            <div class="form-group full-width">
                                <label for="alamat">Alamat</label>
                                <textarea id="alamat" name="alamat"><?php echo htmlspecialchars($pengajar_data['alamat']); ?></textarea>
                            </div>
                            <div class="form-group full-width">
                                <label for="riwayat_pendidikan">Riwayat pendidikan</label>
                                <textarea id="riwayat_pendidikan" name="riwayat_pendidikan"><?php echo htmlspecialchars($pengajar_data['riwayat_pendidikan']); ?></textarea>
                            </div>
                        </div>
                    </div>
                </div>
